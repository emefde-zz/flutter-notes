...
main() {
  group('Weather tests', () {
    ...
    group('fromJson', () {
      test(
          'returns [WeatherState.unknown] for unsupported [weather_state_abbr]',
          () {
        expect(
            weather,
            isA<Weather>().having((weather) => weather.weatherStateAbbr, 'abbr',
                WeatherState.unknown));
      });

      group('test extension [WeatherStateAbbr]', () {
        test('returns proper abbreviation', () {
          final weatherState = WeatherState.clear;
          expect(weatherState.abbr, 'c');
        });
      });
    });
  });
